...
function issueToken(user) {
  return jwt.sign({ email: user.email, name: user.name || undefined }, jwtSecret(), { subject: user.id, expiresIn: '7d' });
}
...
router.get('/me', requireAuth, async (req, res, next) => {
  try {
    ...
    return user ? res.json({ user: publicUser(user) }) : res.status(401).json({ error: 'Account no longer exists' });
  } catch (error) { return next(error); }
});
